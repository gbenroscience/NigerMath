...
public class Grid {
    ...
    public Grid(String function, boolean showGridLines, boolean labelAxis,
            ...
            double xStep, double yStep, Font font, JPanel component) {
        /**
...
// Only trigger if an update was requested while we were locked
    ...
    public void replotOnLimitChange1() {
        //lock for math updates and subsequent graphic updates
        ...
        final Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (lowerVisibleX <= lowerXLimit) {
                    ...
                    System.out.println("Now scrolled to extreme LEFT, rebuilding graph values for continuity...");

                } else if (upperXLimit <= upperVisibleX) {
                    ...
                    System.out.println("Now scrolled to extreme RIGHT, rebuilding graph values for continuity...");
...
//draw to the right of the y axis
                while (x < xLocOfRightSideOfComponent) {
                    ...
                    if (labelAxis) {
                        g.drawString(horizontalAxisLabel.getName() + "→", compSize.width - 80, (int) locationOfOrigin.y + majorTickLength + 40);
                    }
...
//draw the major tick lengths
                    if (countTicks == 0) {
                        ...
                        if (!numLabel.equals("0.0")) {
                            if (labelAxis) {
                                g.drawString(numLabel, x, (int) locationOfOrigin.y + majorTickLength + 10);
                            }
                        }
                    }//end if
                    else {
...
//draw to the right of the y axis
            while (y > yLocOfTopSideOfComponent) {
                ...
                if (labelAxis) {
                    g.drawString(verticalAxisLabel.getName() + "↑", (int) locationOfOrigin.x + majorTickLength + 50, yLocOfTopSideOfComponent + 50);
                }
...
//draw the major tick lengths
                if (countTicks == 0) {
                    ...
                    if (!numLabel.equals("0.0")) {
                        if (labelAxis) {
                            g.drawString(numLabel, (int) locationOfOrigin.x + majorTickLength + 10, y);//thicken the ticks
                        }
                    }
                }//end if
                else {
...
//draw the major tick lengths
                if (countTicks == 0) {
                    ...
                    if (!numLabel.equals("0.0") && labelAxis) {
                        g.drawString(numLabel, (int) locationOfOrigin.x + majorTickLength + 10, y);
                    }
                }//end if
                else {
                    g.setColor(tickColor);
     ...
     * draws the grid
     *
     * @param c the Graphics object used to draw.
     */
    public void draw(Graphics g) {
        ...
        try {
                        ...
                        synchronized (gr) {
                            if (gr.getGraphType() == GraphType.FunctionPlot) {
                                ...
                                while (i < gr.getHorizontalCoordinates().length - 1) {
                                    double xx = gr.getHorizontalCoordinates()[i];
                                    double yy = gr.getVerticalCoordinates()[i];
                                    double xx_next = gr.getHorizontalCoordinates()[i + 1];
                                    double yy_next = gr.getVerticalCoordinates()[i + 1];
                                    int[] screenPoint = this.convertGraphPointToScreenCoords(xx, yy);
                                    int[] screenPoint1 = this.convertGraphPointToScreenCoords(xx_next, yy_next);
                                    g.drawLine(screenPoint[0], screenPoint[1], screenPoint1[0], screenPoint1[1]);
                                    i += 1;
            ...
            art.fillOval(g, (int) locationOfOrigin.x, (int) locationOfOrigin.y, 10, 10);
            drawMajorAxes(g);
