 ...
 * @param formula the formula itself i.e the quantity to the right of the equals sign.
 */
 ...
 */
    private void parseFormulae(String formulae){
      ...
      if(!formulae.substring(0,6).equals("store:")){
        ...
        if(Variable.isVariableString( var ) ){
           ...
           if(indexOfEquals!=-1){
             ...
             JOptionPane.showMessageDialog(null, "The formula lacks a dependent variable...\n" +
                     "An example of a valid formula format is  y=7*x^5 ", "Formula Format Error",JOptionPane.ERROR_MESSAGE);
         setValidFormula(false);
           }
     ...
     JOptionPane.showMessageDialog(null, "Error Occured During Formula Parse:\n" +
                     "Cause:\n" +
                     "Invalid Or No Formula Variable used.\n" +
                     "Use The Format When Storing Your Formula:\n" +
                     "Formula Variable = Formula Expression. e.g:" +
                     "y = a*x^6 ","Formula Variable Error",JOptionPane.ERROR_MESSAGE);
     setValidFormula(false);
        }
