...
public class MathExpressionManager {
    ...
    public MathExpressionManager() {
        ...
        MathExpression dummyInit = new MathExpression("1+1", variableManager);
        storeFunction(dummyInit);
    }
    ...
    public void storeFunction(MathExpression function) {
        //variableManager.getVarStore().putAll(function.getVariableManager().getVarStore());
    ...
    public MathExpression createFunction(String expr) {
        MathExpression f = new MathExpression(expr, variableManager); 
        f.setDRG(1);
        storeFunction(f);
        return f;
        ...
        try {
            ...
            if (mathExpr != null && !mathExpr.isEmpty() && exprCount == 1) {
                if ((f = getFunctionByExpression(mathExpr)) != null) {//optimize function
                     ...
                     */
                    f = new MathExpression("(" + mathExpr + ")", variableManager);
                    f.setDRG(drgStatus);
                    storeFunction(f);
                }//end else
