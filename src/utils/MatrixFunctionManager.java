...
import com.github.gbenroscience.math.matrix.expressParser.MatrixExpression;
...
private ArrayList<MatrixExpression> functions=new ArrayList<MatrixExpression>();

/**
 *
 * @return all function objects stored by this FunctionManager object
 */
    public ArrayList<MatrixExpression> getFunctions() {
        return functions;
    }
/**
 *
 ...
 */
    public void setFunctions(ArrayList<MatrixExpression> functions) {
 ...
 */
    public void storeFunction(MatrixExpression function){
 ...
 */
    public boolean contains(MatrixExpression func){
        return functions.indexOf(func)!=-1;
    }


public MatrixExpression getFunction(MatrixExpression function){
 ...
 */
    public MatrixExpression getFunctionAt(int index){
     ...
     */
public MatrixExpression getFunctionByName(String funcName) throws NullPointerException{
 ...
 */
public MatrixExpression getFunctionByScanner(ArrayList<String>scanner){
    int size=functions.size();
    for(int i=0;i<size;i++){
 ...
 */
public MatrixExpression optimizeFunction(String name){
return getFunctionByName(name);
}

public void formatMemory(){
    int choice =JOptionPane.showConfirmDialog(null,"THIS WILL DISABLE THE OPTIMIZER FEATURE OF THE SOFTWARE FOR ALL FUNCTIONS" +
            "USED BEFORE NOW!!!ARE YOU SURE YOU WISH TO CONTINUE????","THIS ACTION CAN BE FATAL!!TAKE IT ONLY IF YOU ARE SURE",JOptionPane.YES_NO_OPTION);
if(choice==JOptionPane.YES_OPTION){
    functions.clear();
    JOptionPane.showMessageDialog(null,"You Just Disabled The Optimizer For All Past Functions\n" +
            "The Optimizer Will Continue To Work For Functions Entered As From Now On." );
}
else if(choice==JOptionPane.NO_OPTION){
    JOptionPane.showMessageDialog(null,"Optimizer Still Active." );
}
}
